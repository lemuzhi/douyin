...
func (dao *Dao) GetMessagebyIdAndTime(formUserId uint, toUserId uint, PreMsgTime int64) (message []model.Message, err error) {
	if PreMsgTime == 0 {
		...
		err = dao.db.Debug().Where("(from_user_id = ? AND to_user_id = ?) OR (to_user_id = ? AND from_user_id = ?)", toUserId, formUserId, toUserId, formUserId).Order("create_time").Find(&message).Error
		return
	}
